...
**Solution**
The command in R Studio for finding the minimum and maximum is very similar to how to find the mean and median, In fact all summary statistics start with df_stats(~variable, data=Data Frame, desired statistics). Here is the command in R Studio for the minimum and maximum of cat's body weight.
...
(ref:density-data-desity-cap) Density Plot of Health Expenditure**

```{r density-data-desity, fig.cap="(ref:density-data-desity-cap)"}
gf_density(~time, data=Nerve)
```

 From the graph (Figure \@ref(fig:desnity-data-desnity)), the data appears to be skewed right. Most of the time between successive nerve pulses appear to be around 5 or 10 1/50 second, but there are some times that are 60 1/50 second.
